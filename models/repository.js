...
class Repository {
    ...
    getAll(params = null) {
        ...
        if (params) {
                ...
                if (paramName == "sort") {
                    ...
                    if (Array.isArray(keyValues)) {
                        for (let key of keyValues) {
                            ...
                            sortKeys.push({ key: values[0], ascending: !descendant });
                        }
                    } else {
                        let value = keyValues.split(',');
                        let descendant = (value.length > 1) && (value[1] == "desc");
                        sortKeys.push({ key: value[0], ascending: !descendant });
                    }
                } else {
                    // todo add search key
            ...
            if(searchKeys.length != 0) {
                filteredAndSortedObjects = this.filterTab(filteredAndSortedObjects, searchKeys);
            }

            // todo sort
            if(sortKeys.length != 0) {
               filteredAndSortedObjects = this.sortTab(filteredAndSortedObjects, sortKeys);
            }
    ...
    getId() {
        this.objects();
        return this.objectsList.map(item => item.Id);
    }
    get(id) {
        for (let object of this.objects()) {
            if (object.Id === id) {
    ...
    filterTab(nonFilteredTab, searchKeys) {
        let tempTab = [];
        let isAMatch = null;
        tempTab = nonFilteredTab.filter(elem => {
            searchKeys.forEach(item => {
                isAMatch = true;
                if(!this.valueMatch(elem[item.key], item.value))
                    isAMatch = false; 
            });
            return isAMatch;
        });
    ...
    sortTab(filteredTab, sortKeys) {
        let tempTab = [];
        this.sortFields = sortKeys;

        tempTab = filteredTab.sort((itemX, itemY) => this.compare(itemX, itemY));
